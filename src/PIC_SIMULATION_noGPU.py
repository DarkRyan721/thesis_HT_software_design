...
with open("data_files/particle_simulation.pkl", "rb") as f:
    data = pickle.load(f)

positions_list = data["positions"]
#velocities_list = data["velocities"]
labels_list = data["labels"]  # si tenías etiquetas, etc.
# num_frames, num_particles, _ = all_positions.shape
num_frames = len(positions_list)
print(f"Se cargaron {num_frames} frames.")
...
# --------------------------------------------------------------------------------
# 4) Crear la nube de partículas inicial (frame 0)
first_positions = positions_list[0]  # (N_0, 3)
first_labels    = labels_list[0]     # (N_0,)

particles = pv.PolyData(first_positions)
particles["ion_label"] = first_labels  # asignamos las etiquetas en un array escalar

particle_actor = plotter.add_mesh(
    particles,
    #scalars="ion_label",         # nombre del campo escalar         # colormap
    point_size=2.0,
...
particles = pv.PolyData(positions_list[0])
...
plotter.add_text("\nHall Effect Thruster", position="upper_edge", color='black')
...
# Bucle que avanza frame a frame
for frame in range(num_frames):
    # Si el usuario cierra la ventana en mitad de la animación, salimos del for
    if window_closed:
        break

    coords = positions_list[frame]
    lbls   = labels_list[frame]

    particles.points = coords
    particles["ion_label"] = lbls
    particle_actor.mapper.dataset.points = coords
    particle_actor.mapper.dataset.point_data["ion_label"] = lbls

    plotter.update()
    time.sleep(1/50)
    print(f"\rFrame: {frame+1}/{num_frames}", end='', flush=True)

print("\nSe han dibujado todos los frames. Esperando a que el usuario cierre la ventana...")

# Bucle adicional para mantener la última vista en pantalla
# mientras la ventana permanezca abierta
while not window_closed:
    plotter.update()
    time.sleep(0.1)

print("Ventana cerrada. Fin de la animación.")
