...
def initialize_particles(N):
    # Generación uniforme en un cubo definido
    r = np.sqrt(np.random.uniform(Rin**2, Rex**2, N))
    theta = np.random.uniform(0, 2*np.pi, N)
    z = np.random.uniform(L, L+40, N)
...
def move_particles(s, v, dt, q_m, E, B0):
    ...
    #     x_new = np.random.uniform(30, 40, num_reinsert)
    #     y_new = np.random.uniform(30, 40, num_reinsert)
    #     z_new = np.random.uniform(65, 75, num_reinsert)  # Z siempre en 180
...
def update(frame):
    ...
    s_gpu = move_particles(s_gpu, v_gpu, dt, q_m, E_gpu, B0)
    s_np = cp.asnumpy(s_gpu)

    scat._offsets3d = (s_np[:, 0], s_np[:, 1], s_np[:, 2])
    ax.set_title(f"Frame: {frame}")
    return scat,

# Ejecutar animación
ani = FuncAnimation(fig, update, frames=tqdm(range(timesteps), desc="Simulando partículas"),
